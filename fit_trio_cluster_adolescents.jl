...
# remember to change the focal individual in the direct model

cd("/ess/p471/cluster/projects/cme_trio")
...
println("Trios number after removing relatives")
...
# -------------------------------------------
    ...
    vd = var(y)
    ini = sqrt.(vd .* [0.2, 0.0, 0.0, 0.1, 0.0, 0.2, 0.7^2]) 
...
# -------------------------------------------
    ...
    dat = VCData(y, X, [rs[1], rs[3], rs[5], rs[7]])
    lbs = [0.0, -Inf, 0.0, 0.0]
    vd = var(y)
    ini = sqrt.(vd .* [0.1, 0.0, 0.1, 0.7^2]) 
    @time mod = VCModel(dat, ini, lbs, false)
   # mod.opt.ftol_abs = 0
   # mod.opt.ftol_rel = 0 
    @time VCModels.fit!(mod)
    hessian!(mod.opt.H, mod)
    JLD.save("results/" * vari * "_mo_$(size(mobadat, 1)).jld", "model", mod)
...
# -------------------------------------------
    ...
    dat = VCData(y, X, [rs[2], rs[3], rs[6], rs[7]])
    lbs = [0.0, -Inf, 0.0, 0.0] 
    vd = var(y)
    ini = sqrt.(vd .* [0.0, 0.0, 0.3, 0.7^2])
    @time mod = VCModel(dat, ini, lbs, false)
    mod.opt.ftol_abs = 0
    mod.opt.ftol_rel = 0 
    @time VCModels.fit!(mod)
    hessian!(mod.opt.H, mod)
    JLD.save("results/" * vari * "_fo_$(size(mobadat, 1)).jld", "model", mod)
...
dat = VCData(y, X, [rs[3], rs[7]]) # changed to 2 for fathers, use 3 for child, and 1 for mother (if focal individual is changed)
lbs = [0.0, 0.0]
